...
# Plot the GFP
fig, ax = plt.subplots()
mne.viz.plot_compare_evokeds(evokeds_to_plot, show_sensors=False, combine='gfp', axes=ax, ylim=dict(eeg=[0, 2]), title='Grand Field Power')
title = 'Grand Field Power'
ax.set_title(title, fontsize=22, pad=20, loc='center')
# Highlight a specific timeframe
highlight_start = 0.195  # in seconds
highlight_end = 0.210  # in seconds
ax.axvspan(highlight_start, highlight_end, color='#A9E2F3', alpha=0.3)
plt.show()
...
channel = 'F3'
fig, ax = plt.subplots()
mne.viz.plot_compare_evokeds(evokeds_to_plot, picks=channel, show_sensors=True, ylim=dict(eeg=[-1.6, 1.7]), axes=ax, show=False, title="", legend=False)

# Set the plot title
title = '{}'.format(channel)
ax.set_title(title, fontsize=23, pad=20, loc='center')

# Highlight a specific timeframe
highlight_start = 0.308  # in seconds
highlight_end = 0.5   # in seconds
ax.axvspan(highlight_start, highlight_end, color='orange', alpha=0.3)
plt.show()


### Get max and min values of the conditions in a specific timeframe ###
...
### GFP ###
# Extract the values within the specified timeframe and find the max values
max_gfp_values = {}
peak_latencies = {}
...
### Get amplitude measurements per subject for an electrode of interest ###
...
#Use timeframe indicated in PCT
tmin = 0.120
tmax = 0.314
channels=['Cz']
results=[]
